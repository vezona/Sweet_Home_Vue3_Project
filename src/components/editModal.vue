          ...
          <div class="row">
            <div class="col-sm-4">
              <div class="mb-1">
                ...
                <!-- 這邊是判斷如果temProduct中的imagesUrl是陣列的話(有多張圖片) -->
                <div v-if="Array.isArray(temProduct.imagesUrl)">
                  <div
                    class="form-group"
                    v-for="(img, index) in temProduct.imagesUrl"
                    :key="index"
                  >
                    <br />
                    ...
                    <input
                      type="text"
                      v-model="temProduct.imagesUrl[index]"
                      :id="`image${index}`"
                      class="form-control"
                      placeholder="請輸入圖片連結"
                    />
                  ...
                  >
                    ...
                    >
                      <!-- 這邊的push是建立一個新的空物件，因為上面的input框是用temProduct去跑回圈 -->
                      <!-- 所以如果建立新的空物件，就會多出現一個輸入框 -->
                      新增圖片
                    </button>
                  </div>

                  <!-- 新增或編輯的"新增紐"需要分開，但是刪除扭只要一個，是因為一旦push進去temProduct的陣列中，這個迴圈就會跑，就帶有刪除扭了 -->
                  <div v-else>
                    <button
              ...
              <div class="row">
                <div class="form-group col-md-6">
                  ...
                  <input
                    ...
                    class="form-control"
                    placeholder="請輸入原價"
                    v-model.number="temProduct.origin_price"
                  />
                </div>
                <div class="form-group col-md-6">
                  ...
                  <input
                    ...
                    class="form-control"
                    placeholder="請輸入售價"
                    v-model.number="temProduct.price"
                  />
                </div>
              </div>
              <hr />
...
export default {
  ...
  //  mixins: [modalMixin],
  props: ['isNew', 'temp', 'apiUrl', 'apiPath'],
  mounted () {
    // 編輯或新增的Modal
  ...
  methods: {
    ...
    updateProduct () {
      ...
      if (this.isNew === false) {
        editProductURL = `${this.apiUrl}/api/${this.apiPath}/admin/product/${this.temProduct.id}`
        mode = 'put'
      }

      // 新建商品  [API]: /api/:api_path/admin/product ； POST
      // axios 的post 跟put 需要傳入data資料，才知道誰是tempProduct
      this.$http[mode](editProductURL, {
